...
def combine_images(filenames, outpath):
    raw_images = [Image.open(fname) for fname in filenames]
    if not len(raw_images): return
    w, h = raw_images[0].size
    
    images = []
    for img in raw_images:
        images.append(img.resize((w, h), Image.BILINEAR))

    width = w * len(images)
    combined = Image.new(images[0].mode, (width, h))
    for i, img in enumerate(images):
        combined.paste(img, box=(i * w, 0))
